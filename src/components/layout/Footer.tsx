    ...
    <footer className="border-t border-white/5 bg-[#030014] pt-20 pb-10">
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8 mb-16">
          <Stack direction="col" className="lg:col-span-2">
            <Link
              href="/"
            ...
            <p className="text-slate-400 text-sm max-w-xs leading-relaxed mb-6">
              The foundational financial operating system for high-performance, decentralized institutions.
            </p>
          </Stack>

          <Stack direction="col" className="gap-6">
            <h4 className="text-white font-semibold">Product</h4>
            <ul className="flex flex-col gap-4">
              {footerNavItems.product.map((item) => (
                <li key={item.title}>
                  <Link href={item.href} className="text-sm text-slate-400 hover:text-[#06b6d4] transition-colors">
                    {item.title}
                  </Link>
                </li>
              ))}
        ...
        <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-sm text-slate-500">
            © {new Date().getFullYear()} Codelinear Systems. All rights reserved.
          </p>
          <div className="flex items-center gap-6 flex-wrap">
            {footerNavItems.legal.map((item) => (
              <Link key={item.title} href={item.href} className="text-sm text-slate-500 hover:text-white transition-colors">
                {item.title}
              </Link>
            ))}
          </div>
