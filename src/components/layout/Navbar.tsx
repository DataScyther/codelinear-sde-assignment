  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? "bg-[#030014]/80 backdrop-blur-md border-white/[0.04] py-3.5"
          : "bg-transparent py-5"
      )}
    >
      <Container className="flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center">
          <Link
            href="#features"
            className="px-4 py-2 rounded-lg bg-white/[0.05] hover:bg-white/[0.08] border border-white/[0.08] text-sm font-medium text-white transition-all"
          >
            Get Started
          </Link>
        </div>
      ...
      {/* Mobile menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#030014]/95 backdrop-blur-xl border-b border-white/[0.04] py-4 px-4 flex flex-col gap-1">
          {headerNavItems.map((item) => (
            <Link
              key={item.title}
              href={item.href}
          ...
          <div className="pt-3 mt-2 border-t border-white/[0.04]">
            <Link
              href="#features"
              className="flex items-center justify-center w-full px-4 py-3 rounded-xl bg-gradient-to-r from-[#a855f7] to-[#06b6d4] text-white text-sm font-medium"
              onClick={() => setIsMobileMenuOpen(false)}
            >
              Get Started
            </Link>
